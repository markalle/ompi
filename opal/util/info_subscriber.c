...
// The below is a cut-and-paste of 
//   OPAL_LIST_RELEASE(list);
// but with an extra free() added because each list item "it" doesn't
// just come frmo an OBJ_NEW(), each also contains a field that was
// strdup()ed.
        do {
            opal_list_item_t *it;
            if (1 == ((opal_object_t*)(list))->obj_reference_count) {
              while (NULL != (it = opal_list_remove_first(list))) {
                  opal_callback_list_item_t *item = it;
                  if (item->default_value) {
                      free(item->default_value); // came from a strdup()
                  }
                  OBJ_RELEASE(it);
              }
            }
